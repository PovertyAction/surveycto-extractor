...
def test_trace_source_scripted_vs_invalid_fallback(tmp_path):
    ...
    _, bad = _run_trace(
        tmp_path / "bad", form, provider=ScriptedProvider({"color": "99"})
    )
...
def test_trace_is_write_only_csv_unchanged(tmp_path):
    ...
    b.mkdir()
    qp_a = a / "q.json"
    qp_a.write_text(json.dumps(form), encoding="utf-8")
    out_a = a / "s.csv"
    generate_synthetic_csv(
        qp_a, out_a, pulldata_search_dirs=[], survey_name="t", n_rows=5, seed=4
    )
