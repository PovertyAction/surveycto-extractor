...
Runs instrument-day (main.py: csv/json/sections/synthetic) and Phase 4
(create_variable_dictionaries.py) over the sample form + dataset in a hermetic
tmp dir, asserting the real outputs. Nothing exercised this before, which is
why Phase 4 shipped broken on a fresh env (#25). The pipeline scripts read
module-global config, so we point those globals at tmp copies via monkeypatch.
...
def sample_env(tmp_path, monkeypatch):
    ...
    s.mkdir()
    src = _sample_dir()
    for f in ("Household_Survey.xlsx", "household_survey.dta",
              "baseline_enum_list.csv", "household_preloads.csv"):
        shutil.copy(src / f, s / f)
    out = s / "output"
    out.mkdir()

    surveys = {"household_survey": {
...
def test_instrument_day_and_phase4(sample_env, monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv",
                        ["cvd.py", "--survey", "household_survey", "--xlsx"])
    cvd.main()
