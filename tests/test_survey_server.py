...
_SERVER = Path(__file__).resolve().parents[1] / "mcp_server" / "survey_server.py"
...
class TestInfoPrefersPostXmlCount:
    def test_uses_matched_after_xml_when_present(self, tmp_path):
        store = _store_with_summary(tmp_path, {
            "total_variables": 543, "matched_to_questions": 526,
            "unmatched": 17, "matched_after_xml": 528,
        })
        out = store.get_info()
    ...
    def test_falls_back_to_prexml_count(self, tmp_path):
        store = _store_with_summary(tmp_path, {
            "total_variables": 100, "matched_to_questions": 90, "unmatched": 10,
        })
        out = store.get_info()
