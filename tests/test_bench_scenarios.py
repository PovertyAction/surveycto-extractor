...
_HELPER = (
    Path(__file__).resolve().parents[1] / "skill" / "bench-test" / "bench_scenarios.py"
)
    ...
    bad = _write(
        tmp_path / "bad.json",
        {
            ...
            "scenarios": [
                {
                    "id": "bad",
                    "must_hit": {"consent": "9", "age": 999, "nope": "x"},
                    "expect": {"reach": ["ghost"]},
                },
            ],
        },
...
def test_expand_writes_sheets_and_manifest_deterministically(tmp_path):
    ...
    assert sheets == [
        "alpha_v01.answers.json",
        "alpha_v02.answers.json",
        ...
        _trace(
            {
                "rel_2": {
                    "var": "rel",
                    "asked": True,
                    "source": "scripted_invalid_fallback",
                    "note": "'3' not in choices",
                }
            },
            unevaluable_gates=[
                {"key": "q9", "expr": "bad-fn()", "error": "unsupported function"}
            ],
        ),
    )
    clean = _write(
        tmp_path / "clean.json",
        _trace({"rel_2": {"var": "rel", "asked": True, "source": "scripted"}}),
    )
