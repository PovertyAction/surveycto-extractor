...
`create_variable_dictionaries.py` (Phase 4) maps dataset columns to form questions
...
occasionally mis-match. When the **compiled XForm XML** is available it is the
...
Usage:
    python enrich_with_contract.py --survey KEY     # one dataset
    python enrich_with_contract.py --all            # every dataset with an xml_path
...
# Local module (same directory). Pure stdlib parser, no third-party XML dep.
sys.path.insert(0, str(Path(__file__).parent))
from parsers.xml_contract import parse_contract  # noqa: E402
...
def enrich_contract(
    ...
    if not dict_path.exists():
        raise FileNotFoundError(
            ...
            f"(create_variable_dictionaries.py) first."
        )
...
def main() -> None:
    ...
    g.add_argument("--survey", metavar="KEY", help="one dataset key from config.DATASETS")
    g.add_argument("--all", action="store_true", help="every dataset that has an xml_path")
    ...
    if args.survey:
        if args.survey not in DATASETS:
            print(f"ERROR: '{args.survey}' not in config.DATASETS. "
