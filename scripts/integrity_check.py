...
# Paths a RECIPE creates, gitignored so they never exist in a fresh checkout.
# These are output DESTINATIONS -- `config.example.toml` and `.dep-accept.toml`
# name them so the pipeline can write them -- so an absent one means the recipe
# has not run, which is not a defect. Reported as INFO and never counted.
#
# Missing this class is what made the first version of this sweep pass on my
# machine and fail in CI: `sample/output/` exists locally because the pipeline has
# been run, so its six references resolved, while a clean runner saw six dead
# paths. A sweep that only holds on a dirty tree is measuring the tree, not the
# commit -- and no ablation could have caught it, since the ablations run in
# whatever tree is already there.
GENERATED_PREFIXES: dict[str, str] = {
    "sample/output": "generated by the sample pipeline; gitignored, so absent until it runs",
    "data/_integrity": "this script's own report directory",
}
...
def _generated(ref: str) -> str | None:
    """Return the reason `ref` is a recipe's output destination, or None."""
    for prefix, why in GENERATED_PREFIXES.items():
        if ref.startswith(prefix):
            return why
    return None
...
def sweep_2(rep: Report) -> None:
    ...
    generated = [(r, s, _generated(r)) for r, s in miss if _generated(r)]
    allowed = [
        (r, s, _allowed(r)) for r, s in miss if _allowed(r) and not _generated(r)
    ]
    live = [(r, s) for r, s in miss if not _allowed(r) and not _generated(r)]

    rep.say(
        f"  {len(hits)} distinct repo-relative paths referenced across "
        f"{len(TRACKED)} tracked files"
    )
    rep.say(f"  INFO     {len(generated)} references to a recipe's output destination:")
    for ref, srcs, why in generated:
    ...
    # The sweep's verdict must not depend on whether a recipe has been run in this
    # tree. Every generated prefix classifies the same way whether or not it is on
    # disk -- which is the property whose absence made this pass locally and fail
    # in CI. Asserted directly, since no injected defect can surface it.
    rep.check(
        "ABLATION: generated destinations classify independently of the tree's state",
        all(
            _generated(f"{p}/whatever_it_writes.json") is not None
            for p in GENERATED_PREFIXES
        )
        and _generated("sample/household_survey.dta") is None,
        "",
    )
