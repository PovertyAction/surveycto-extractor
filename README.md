...
- Query the form's structure — gate chains, repeat nesting, choice lists — while
  you write your cleaning code, via the MCP server or the `survey-expert` skill.

To also **dry-run your cleaning + HFC pipeline before the first submission**,
generate a simulated export with [surveycto-deploy-gate](https://github.com/PovertyAction/surveycto-deploy-gate), which is purpose-built for
it. This toolkit used to carry its own generator; that engine now lives there.
...
Simulated exports — a wide CSV shaped exactly like what SurveyCTO will produce
once real submissions arrive, so you can run your cleaning and HFC code before
fieldwork — are generated by [surveycto-deploy-gate](https://github.com/PovertyAction/surveycto-deploy-gate).
