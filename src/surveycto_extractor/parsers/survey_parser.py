...
class SurveyParser:
    ...
    def _normalize_string_cells(df: pd.DataFrame) -> pd.DataFrame:
        r"""Strip whitespace and coerce NaN/None to '' in every string cell.
        ...
        if df is None or df.empty:
            return df
        return df.apply(lambda col: col.where(col.notna(), "").astype(str).str.strip())
    ...
    def load(self) -> tuple[pd.DataFrame, pd.DataFrame]:
        ...
        try:
            ...
            self.survey_df = pd.read_excel(
                self.file_path, sheet_name="survey", dtype=str, keep_default_na=False
            )
            self.survey_df = self._normalize_string_cells(self.survey_df)

            # Load choices sheet
            self.choices_df = pd.read_excel(
                self.file_path, sheet_name="choices", dtype=str, keep_default_na=False
            )
    ...
    def _load_external_choices(self) -> pd.DataFrame:
        ...
        try:
            csv_df = pd.read_csv(
                self.external_choices_csv, dtype=str, encoding="utf-8-sig"
            )
            ...
            value_cols = [
                ...
                if c.endswith("_value") or c.endswith("_key") or c.endswith("_id")
            ]
    ...
    def _merge_choices(
        self, xlsx_choices: pd.DataFrame, external_choices: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        xlsx_lists = (
            set(xlsx_choices["list_name"].str.strip().unique())
            if "list_name" in xlsx_choices.columns
            else set()
        )
