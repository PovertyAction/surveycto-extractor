...
from surveycto_extractor.parsers.group_tracker import GroupStack
from surveycto_extractor.parsers.type_parser import parse_question_type
...
def _parse_refs(*expressions: str | None) -> list[str] | None:
...
def _is_static_choice_filter(expr: str) -> bool:
    """Return True when the choice_filter is static (has no `${var}` reference).
...
def _eval_static_choice_filter(expr: str, choice_row: dict[str, str]) -> bool | None:
...
def _apply_static_choice_filter(
    expr: str | None, choices: list[dict] | None
) -> list[dict] | None:
...
class JSONExtractor:
    ...
    def __init__(
        self, survey_df: pd.DataFrame, choices_df: pd.DataFrame, output_dir: Path
    ):
    ...
    def _build_choice_lookup(self) -> None:
        ...
        filter_cols = [
            c
            for c in self.choices_df.columns
    ...
    def _build_question_type_lookup(self) -> None:
        """Build two dicts used by the logic converter in a pre-pass over the survey sheet.
    ...
    def _is_excluded(self, row: pd.Series) -> bool:
        ...
        return bool(
            variable_name
            and any(
                variable_name.startswith(prefix) for prefix in config.SYSTEM_PREFIXES
            )
        )

    def _extract_question(self, row: pd.Series) -> dict | None:
        ...
        if _blank(question_text):
            ...
            ordered = [
                c for c in label_cols if str(c).lower() in ("label:english", "label:en")
            ] + [
                c
                for c in label_cols
                if str(c).lower() not in ("label:english", "label:en")
            ]
            for c in ordered:
                v = row.get(c, "")
                if not _blank(v):
        ...
        for idx, row in self.survey_df.iterrows():
            ...
            if row_type == "begin group":
                ...
                if group_name.startswith("unnamed_group_"):
                    print(
                        f"  [WARN] begin group at row {idx} has blank `name` cell - using placeholder '{group_name}'"
                    )
                ...
                if repeat_name.startswith("unnamed_repeat_"):
                    print(
                        f"  [WARN] begin repeat at row {idx} has blank `name` cell - using placeholder '{repeat_name}'"
                    )
