...
class PullDataTable:
    ...
    def __init__(self, name: str, df: pd.DataFrame):
        """Store the table name and DataFrame; per-key-column indexes build lazily."""
        self.name = name
        self.df = df
    ...
    def lookup(self, col: str, key_col: str, key_value: Any) -> str:
        """Return ``col`` from the first row where ``key_col`` equals ``key_value`` (else "")."""
    ...
    for name in sorted(refs["static"]):
        ...
        try:
            ...
            raise MissingPullDataError(
                f"pulldata source {name!r} found at {path} but failed to load: {exc}"
            ) from exc
        ...
        for d in search_dirs:
            ...
            for csv_path in d.glob("*.csv"):
                ...
                try:
                    ...
                    print(f"  [pulldata] WARNING: failed to load {csv_path}: {exc}")
