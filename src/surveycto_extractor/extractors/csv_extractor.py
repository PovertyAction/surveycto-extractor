...
class CSVExtractor:
    ...
    def extract_survey_csv(self, columns: list[str], output_name: str) -> Path:
        ...
        print(
            f"[OK] Created {output_name}: {len(df_filtered)} rows, {len(available_columns)} columns"
        )
        return output_path
    ...
    def extract_choices_csv(self, columns: list[str], output_name: str) -> Path:
        ...
        if "list_name" in available_columns:
            ...
            print(
                f"[OK] Created {output_name}: {len(df_filtered)} choices across {len(list_counts)} lists"
            )
        else:
