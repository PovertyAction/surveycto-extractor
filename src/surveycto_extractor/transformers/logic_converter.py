...
class LogicConverter:
    ...
    def _find_balanced(s: str, open_idx: int) -> int:
        r"""Given `s` and the index of an opening `(`, return the index of the
    ...
    def _mask_literals(expr: str):
        r"""Replace every quoted string literal (both ' and " styles) with a
    ...
    def _unwrap_one_quote(s: str) -> str:
        r"""Strip exactly one matching outer quote pair from a needle, leaving
    ...
    def _translate_selected(
        ...
        """Translate a selected(...) call to a Stata expression across three patterns.
    ...
    def _translate_count_selected(
        match: re.Match,
        choice_codes: dict[str, list[str]] | None,
        varname: str,
    ) -> str:
    ...
    def _clean_sentinels(expr: str) -> str:
        ...
        while prev != expr:
            ...
            expr = re.sub(rf"\w+\s*\([^()]*{_SEN}[^()]*\)", _SENTINEL, expr)
            ...
            expr = re.sub(rf"{_ARITH_OPERAND}\s*[+\-*/]\s*{_SEN}", _SENTINEL, expr)
            expr = re.sub(rf"{_SEN}\s*[+\-*/]\s*{_ARITH_OPERAND}", _SENTINEL, expr)
            # Strip _SENTINEL with adjacent comparison: `_SENTINEL op operand`
            expr = re.sub(rf"{_SEN}\s*{_OP}\s*{_OPERAND}", _SENTINEL, expr)
            # Mirror: `operand op _SENTINEL`
            expr = re.sub(rf"{_OPERAND}\s*{_OP}\s*{_SEN}", _SENTINEL, expr)
    ...
    def convert_to_stata(
        surveycto_expr: str | None,
        question_types: dict[str, str],
        choice_codes: dict[str, list[str]] | None = None,
        varname: str = "",
    ) -> str | None:
        ...
        expr = re.sub(
            r"\bstring\s*\(([^)]+)\)", _strip_string_literal, expr, flags=re.IGNORECASE
        )
        ...
        expr = LogicConverter._sub_outside_literals(
            re.compile(r"(\w+)\s*(!=|==?)\s*'([^']+)'"), _single_quoted_cmp, expr
        )
        ...
        expr = re.sub(
            r"\bempty\s*\(\s*(\w+)\s*\)", r"missing(\1)", expr, flags=re.IGNORECASE
        )
        ...
        expr = re.sub(
            r"\brelevant\s*\(\s*(\w+)\s*\)", r"!missing(\1)", expr, flags=re.IGNORECASE
        )
        ...
        expr = re.sub(
            r"(?<![\w-])number\s*\(\s*([^)]+?)\s*\)", r"\1", expr, flags=re.IGNORECASE
        )
        ...
        expr = LogicConverter._sub_function_balanced(
            expr, r"coalesce", _coalesce_replacer
        )
        ...
        expr = LogicConverter._sub_function_balanced(
            expr, r"contains", _contains_replacer
        )
        ...
        expr = LogicConverter._sub_function_balanced(
            expr, r"starts-with", _starts_with_replacer
        )
        ...
        expr = LogicConverter._sub_function_balanced(
            expr, r"ends-with", _ends_with_replacer
        )
        ...
        expr = re.sub(
            r"\bsubstr\s*\(([^)]+)\)",
            LogicConverter._translate_substr,
            expr,
            flags=re.IGNORECASE,
        )
        ...
        expr = re.sub(
            r"\bcount-selected\s*\(\s*(\w+)\s*\)", _cs_sub, expr, flags=re.IGNORECASE
        )
        ...
        expr = LogicConverter._sub_function_balanced(
            expr, r"indexed-repeat", _ir_replacer
        )
        ...
        expr = LogicConverter._strip_balanced(
            expr, r"count-selected", varname, "COUNT_SEL_NO_CODES"
        )
        ...
        def _sel_replacer(args: str, full_call: str) -> str:
            return LogicConverter._translate_selected(
                args, full_call, question_types, varname
            )
        ...
        expr = LogicConverter._strip_balanced(
            expr, r"selected", varname, "DYNAMIC_SELECTED"
        )
        ...
        expr = LogicConverter._strip_balanced(
            expr, r"position|index", varname, "POSITION"
        )
        ...
        expr = LogicConverter._strip_balanced(
            expr, r"jr:choice-name", varname, "CHOICE_NAME"
        )
        expr = LogicConverter._strip_balanced(
            expr, r"choice-label", varname, "CHOICE_NAME"
        )
        expr = LogicConverter._strip_balanced(
            expr, r"selected-at", varname, "SELECTED_AT"
        )
    ...
    def structural_issues(condition: str) -> list[str]:
        ...
        # too; normalize before the allowlist check. (review #12)
        for m in re.finditer(r"\b([a-z][a-z0-9:-]*)\s*\(", bare, flags=re.IGNORECASE):
            name = m.group(1).lower()
            if (
                name not in LogicConverter._EMIT_WHITELIST
    ...
    def validate_translations(conditions: list) -> None:
        ...
        CHECKS = [
            ...
            (
                "today()/now()",
                re.compile(
                    r"\b(?:today|now|date|date-time|decimal-time|decimal-date-time|format-date-time)\s*\(",
                    re.IGNORECASE,
                ),
            ),
    ...
    @staticmethod
    def combine_conditions(conditions: list[str], operator: str = "&") -> str | None:
    ...
    def convert_with_inheritance(
        question_relevance: str | None,
        group_relevances: list[str | None],
        question_types: dict[str, str],
        choice_codes: dict[str, list[str]] | None = None,
        varname: str = "",
    ) -> str | None:
