...
Usage:
    uv run surveycto-vardict [--survey KEY] [--validate] [--xlsx]
...
# (see config_loader). None/empty when absent (tests, CI) so the module still
# imports; main() fails with a clear message if DATASETS is empty when run.
from surveycto_extractor.config_loader import load_config
from surveycto_extractor.transformers.logic_converter import LogicConverter

config = load_config()
DATASETS = getattr(config, "DATASETS", {}) if config is not None else {}
try:
    from surveycto_extractor.core import sentinels as _sentinels
    ...
    def _fb_meanings(config=None):
        ...
        return (
            {int(k): tuple(v) for k, v in ov.items()}
            if ov is not None
            else dict(_DEFAULT_MEANINGS)
        )
    ...
    print(
        "[WARN] core/sentinels.py not found -- using built-in IPA default sentinel "
        "table (-99=.d, -88=.r, -77=.n, -66=.o, -55=.m; scan-only -98). "
        "Override via config.SENTINEL_MEANINGS. See README."
    )


def _write_parquet_sidecar(df: pd.DataFrame, dta_path: Path) -> Path | None:
    ...
    print(f"  Wrote parquet sidecar: {pq_path.name} ({size_mb:.1f} MB, {elapsed:.1f}s)")
    return pq_path


def _read_parquet_column_stats(pq_path: Path | None) -> dict[str, dict]:
    ...
    _numeric_or_temporal = {
        ...
        if (
            _pat.is_integer(t)
            or _pat.is_floating(t)
            or _pat.is_decimal(t)
            or _pat.is_date(t)
            or _pat.is_timestamp(t)
            or _pat.is_time(t)
        )
    }
    ...
    for col_idx, name in enumerate(col_names):
        ...
        if col_idx in _null_typed:
            ...
            if null_count is not None or num_values is not None:
                ...
                entry["missing_rate"] = (
                    round(null_count / total_rows, 6) if total_rows > 0 else 0.0
                )
    ...
    print(
        f"  Parquet column stats: {len(result)} columns, {n_minmax} with min/max ({elapsed:.1f}s)"
    )
    return result
...
def _scan_and_clean_extended_missings(
    ...
    if not obj_cols:
        print(
            f"  Extended missing scan+clean: 0 object cols, skipped ({time.perf_counter() - t0:.1f}s)"
        )
        return df, result
    ...
    print(
        f"  Extended missing scan+clean: {n_vars} vars with {n_total:,} "
...
def load_data(dataset_name: str):
    ...
    if not questions_path.exists():
        raise FileNotFoundError(
            ...
            f"  Run: uv run surveycto-extract --survey {dataset_name} --phases json"
        )
    ...
    if _PYREADSTAT_AVAILABLE:
        ...
        elapsed = time.perf_counter() - t0
        print(
            f"  Loaded {len(df)} obs x {len(df.columns)} vars "
        ...
        cat_cols = [
            c for c in df.columns if isinstance(df[c].dtype, pd.CategoricalDtype)
        ]
        if cat_cols:
            raise TypeError(
    ...
    with open(cfg[json_key], encoding="utf-8") as f:
...
def batch_sentinel_scan(
    ...
    ext_missing_counts: dict[str, dict[str, int]] | None = None,
) -> pd.DataFrame:
    ...
    numeric_cols = [
        c
        for c in df.columns
        if pd.api.types.is_numeric_dtype(df[c]) and c in var_to_idx
    ]
    if numeric_cols:
        sentinel_counts = df[numeric_cols].isin(SENTINEL_CODES_LIST).sum()
        for col, cnt in sentinel_counts[sentinel_counts > 0].items():
            idx = var_to_idx[col]
            var_dict_df.iat[idx, var_dict_df.columns.get_loc("sentinel_raw_int")] = int(
                cnt
            )
            detail = df[col][df[col].isin(SENTINEL_CODES_LIST)].value_counts()
            var_dict_df.iat[idx, var_dict_df.columns.get_loc("sentinel_raw_detail")] = (
                json.dumps({str(int(k)): int(v) for k, v in detail.items()})
            )
    ...
    if string_cols:
        ...
        for start in range(0, len(string_cols), chunk_size):
            ...
            for col, cnt in counts[counts > 0].items():
                idx = var_to_idx[col]
                var_dict_df.iat[idx, var_dict_df.columns.get_loc("sentinel_string")] = (
                    int(cnt)
                )
                vals = chunk_df[col][sentinel_mask[col]].value_counts()
                var_dict_df.iat[
                    idx, var_dict_df.columns.get_loc("sentinel_string_detail")
                ] = json.dumps({str(k): int(v) for k, v in vals.items()})
    ...
    if ext_missing_counts:
        for col, counts in ext_missing_counts.items():
            if col in var_to_idx:
                ...
                var_dict_df.iat[
                    idx, var_dict_df.columns.get_loc("sentinel_ext_missing")
                ] = total
                var_dict_df.iat[
                    idx, var_dict_df.columns.get_loc("sentinel_ext_missing_detail")
                ] = json.dumps(counts)
    ...
    for idx, row in var_dict_df.iterrows():
        ...
        if qtype in ("integer", "decimal") and (
            "str" in str(stype) or "object" in str(stype)
        ):
            var_dict_df.at[idx, "sentinel_type_mismatch"] = True

    # --- State 4: calculate fields with unexplained negatives ---
    calc_vars = var_dict_df[var_dict_df["question_type"] == "calculate"][
        "variable_name"
    ].tolist()
    calc_numeric = [
        c for c in calc_vars if c in df.columns and pd.api.types.is_numeric_dtype(df[c])
    ]
    if calc_numeric:
        for col in calc_numeric:
            ...
            if neg_count > 0:
                idx = var_to_idx[col]
                var_dict_df.iat[
                    idx, var_dict_df.columns.get_loc("sentinel_calculate_risk")
                ] = True
...
def _build_question_index(questions: list[dict]) -> QuestionIndex:
    ...
    for prefix, bucket in trunc32.items():
        if len(bucket) > 1:
            ...
            print(
                f"  [WARN] 32-char prefix '{prefix}' shared by {len(bucket)} form "
    ...
    for low, bucket in lower.items():
        if len(bucket) > 1:
            names = [q["variable_name"] for q in bucket]
            print(
                f"  [WARN] lowercase name '{low}' shared by {len(bucket)} form "
...
def _question_in_repeat(question: dict, repeat_group_names: set) -> bool:
    """Return True if any segment of the question's group_path is a repeat group."""
    gp = question.get("group_path") or []
    return any(seg in repeat_group_names for seg in gp)


def _resolve_unambiguous(bucket_map: dict[str, list[dict]], key: str) -> dict | None:
...
def find_question_for_variable(
    var_name: str, questions: list[dict], _index: QuestionIndex | None = None
) -> dict | None:
...
def get_choice_label(question: dict, choice_code: str) -> str | None:
...
def adjust_variable_refs(
    logic_str: str,
    repeat_groups: set,
    iteration: int,
    questions: list[dict],
    _index: QuestionIndex | None = None,
) -> str:
...
def adjust_skip_logic_for_repeats(
    metadata: dict,
    var_name: str,
    questions: list[dict],
    _index: QuestionIndex | None = None,
    ...
    rgs = (
        _index.repeat_group_names
        if _index is not None
        else _repeat_group_names(questions)
    )
    ...
    if metadata["group_relevances"]:
        ...
        for relevance in metadata["group_relevances"]:
            adjusted = adjust_variable_refs(
                relevance, repeat_groups, iteration, questions, _index=_index
            )
            adjusted = replace_index_function(adjusted, iteration)
            converted = LogicConverter.convert_to_stata(
                adjusted, types, codes, var_name
            )
...
def get_special_code_meaning(choice_code: str) -> str | None:
...
def determine_variable_source(
    ...
    degraded = (
        _index.repeat_detection_degraded
        if _index is not None
    ...
    def _mark_select_multiple(code: str) -> None:
        ...
        result["choice_index"] = (
            choice_values.index(code) if code in choice_values else None
        )
    ...
    if var_name != base_name:
        ...
        if double_match:
            ...
            match_direct = (
                re.match(rf"{re.escape(base_name)}(\d+)$", var_name)
                if not match
                else None
            )
    ...
    if result["is_repeat"]:
        result = adjust_skip_logic_for_repeats(
            result, var_name, questions, _index=_index
        )
...
def enrich_count_variables(var_dict_df: pd.DataFrame, df: pd.DataFrame) -> pd.DataFrame:
    ...
    for idx in var_dict_df[repeat_count_mask].index:
        ...
        repeat_group_name = (
            group_path.split("/")[-1]
            if pd.notna(group_path) and group_path
            else base_name
        )
        ...
        if related:
            print(
                f"    {var_name}: {len(related)} related variables in group '{repeat_group_name}'"
            )
...
def create_variable_dictionary(
    ...
    dataset_name: str,
    ext_missing_counts: dict[str, dict[str, int]] | None = None,
    minmax: dict[str, dict] | None = None,
    meta=None,
) -> pd.DataFrame:
    ...
    for i, var_name in enumerate(df.columns, 1):
        ...
        record = {
            ...
            "group_relevances": json.dumps(metadata["group_relevances"])
            if metadata["group_relevances"]
            else None,
            "group_path": metadata["group_path"],
            ...
            "group_relevances_template": json.dumps(
                metadata["group_relevances_template"]
            )
            if metadata["group_relevances_template"]
            else None,
            "group_relevances_iteration_specific": json.dumps(
                metadata["group_relevances_iteration_specific"]
            )
            if metadata["group_relevances_iteration_specific"]
            else None,
            ...
            "references": json.dumps(metadata["references"])
    ...
    for _, row in var_dict_df.iterrows():
        ...
        sort_choice.append(
            float(row["choice_index"]) if pd.notna(row["choice_index"]) else 0.0
        )
    ...
    var_dict_df = var_dict_df.sort_values(
        ["_sort_group", "_sort_iter", "_sort_within", "_sort_choice"], kind="stable"
...
def save_ord_dta(
    ...
    for col in df_ord.select_dtypes(include=["object", pd.StringDtype()]).columns:
        with contextlib.suppress(ValueError, TypeError):
            # leave as a string column when it doesn't parse as numeric
    ...
    if meta is not None:
        if hasattr(meta, "column_names_to_labels") and meta.column_names_to_labels:
            ...
            col_labels_list = [
                str(name_to_label.get(c, "") or "") for c in ordered_cols
            ]
        if hasattr(meta, "variable_value_labels") and meta.variable_value_labels:
            val_labels = {
                k: v for k, v in meta.variable_value_labels.items() if k in ordered_cols
            }
...
def export_dictionary(
    ...
    print(
        f"  Repeat variables with iteration-specific skip logic: {var_dict['skip_logic_iteration_specific'].notna().sum()}"
    )

    # Missing-rate diagnostics from parquet metadata
    if "missing_rate" in var_dict.columns and var_dict["missing_rate"].notna().any():
        all_missing = var_dict[var_dict["is_all_missing"]]
        sparse = var_dict[
            var_dict["missing_rate"].notna()
            & (var_dict["missing_rate"] >= 0.95)
    ...
    for _, row in var_dict.iterrows():
        ...
        group_relevances = (
            json.loads(row["group_relevances"])
            if pd.notna(row["group_relevances"])
            else None
        )
        group_relevances_template = (
            json.loads(row["group_relevances_template"])
            if pd.notna(row["group_relevances_template"])
            else None
        )
        group_relevances_iteration_specific = (
            json.loads(row["group_relevances_iteration_specific"])
            if pd.notna(row["group_relevances_iteration_specific"])
            else None
        )
        repeat_metadata = (
            json.loads(row["repeat_metadata"])
            if pd.notna(row["repeat_metadata"])
            else None
        )
        ...
        var_metadata = {
            ...
            "survey": {
                "original_variable_name": row["original_variable_name"]
                if pd.notna(row["original_variable_name"])
                else None,
                ...
                if pd.notna(row["disabled"])
                else False,
                "choice_list": row["choice_list"]
                if pd.notna(row["choice_list"])
                else None,
                "choices": choices,
                "stata_skip_logic": row["stata_skip_logic"]
                if pd.notna(row["stata_skip_logic"])
                else None,
                "group_relevances": group_relevances,
                "group_path": row["group_path"]
                if pd.notna(row["group_path"])
                else None,
            },
        }
        ...
        if pd.notna(row["skip_logic_iteration_specific"]):
            var_metadata["survey"]["skip_logic_iteration_specific"] = row[
                "skip_logic_iteration_specific"
            ]
        if group_relevances_template:
            var_metadata["survey"]["group_relevances_template"] = (
                group_relevances_template
            )
        if group_relevances_iteration_specific:
            var_metadata["survey"]["group_relevances_iteration_specific"] = (
                group_relevances_iteration_specific
            )
        ...
        if int(row.get("sentinel_ext_missing", 0)) > 0:
            ...
            if pd.notna(row.get("sentinel_ext_missing_detail")):
                sentinels["ext_missing_detail"] = json.loads(
                    row["sentinel_ext_missing_detail"]
                )
        ...
        if missing_rate is not None and not (
            isinstance(missing_rate, float) and np.isnan(missing_rate)
        ):
        ...
        if d_min is not None and d_max is not None and qtype in _MINMAX_QUESTION_TYPES:
            try:
    ...
    output_structure = {
        ...
        "summary": {
            ...
            "matched_to_questions": int(
                var_dict["original_variable_name"].notna().sum()
            ),
    ...
    try:
        ...
        print(
            f"[WARN] _ord.dta generation failed for {dataset_name}: {exc}\n"
    ...
    for var_name, survey_info in candidates:
        ...
        if not original_question:
            mismatches.append(
                {
                    "variable": var_name,
                    "issue": "Original question not found",
...
def _build_repeat_tree(
    questions: list[dict], variables: dict, repeat_group_names: set
) -> dict:
    ...
    for rg_name, q in repeat_counts.items():
        ...
        parts = (
            ...
            else [p.strip() for p in (gp or "").replace("/", " ").split() if p.strip()]
        )
        parent = None
    ...
    for rg_name in sorted(repeat_counts.keys()):
        ...
        if d == 1:
            join_note = (
                "Iteration index is the only key -- "
                "suffix _N maps directly to roster position N."
            )
        else:
            join_note = (
                f"Nested repeat -- Stata columns have suffix pattern "
...
def build_variable_graph(
    questions: list[dict], vardict_json: dict, output_path: Path
) -> Path | None:
    ...
    def _repeat_info(group_path):
        ...
        parts = (
            ...
            else [p.strip() for p in group_path.replace("/", " ").split() if p.strip()]
        )
    ...
    print(
        f"  Variable graph: {G.number_of_nodes()} nodes, "
...
def main():
    """Run the vardict CLI entry point."""
    ...
    parser.add_argument(
        "--validate", action="store_true", help="Run validation after creation"
    )
    parser.add_argument(
        "--validate-only",
        action="store_true",
    ...
    if args.validate_only:
        ...
        if args.survey not in DATASETS:
            print(
                f"ERROR: '{args.survey}' not found in config.DATASETS. "
        ...
        print(
            "ERROR: no survey selected. Pass --survey KEY to process one "
    ...
    for dataset_name in datasets_to_process:
        ...
        print(f"{'=' * 70}")
        df, questions, cfg, meta, pq_path, ext_missing_counts, minmax = load_data(
            dataset_name
        )
        var_dict = create_variable_dictionary(
            df, questions, dataset_name, ext_missing_counts, minmax, meta=meta
        )
        ...
        if not args.no_enrich_xml:
            try:
                from surveycto_extractor.cli.enrich import enrich_dataset

                enrich_dataset(dataset_name, cfg)
            except Exception as exc:  # noqa: BLE001 — enrichment is additive, never fatal
                print(
                    f"[WARN] XML-contract enrichment skipped for {dataset_name}: {exc}"
                )
        ...
        graph_path = vardict_path.with_name(
            vardict_path.stem.replace("_variable_dictionary", "_variable_graph")
            + ".json"
        )
        ...
        if args.xlsx:
            from surveycto_extractor.generators.xlsx_exporter import XLSXExporter
