...
Usage (via surveycto-vardict --xlsx):
...
class XLSXExporter:
    ...
    def export(self, var_dict_json_path: Path, output_path: Path) -> None:
        """Load a variable-dictionary JSON and write a formatted Excel workbook."""
        var_dict_json_path = Path(var_dict_json_path)
        output_path = Path(output_path)

        print(f"  Loading JSON: {var_dict_json_path}")
        with open(var_dict_json_path, encoding="utf-8") as f:
            data = json.load(f)
        ...
        sorted_vars = sorted(
            variables.items(), key=lambda kv: kv[1].get("variable_order", 0)
        )
        ...
        for row_idx, (var_name, var_info) in enumerate(sorted_vars, start=2):
            ...
            row_values = {
                ...
                "skip_logic_iteration_specific": survey.get(
