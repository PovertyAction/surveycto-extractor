...
could not interpret. CLI in ``surveycto-extract`` exposes ``--phases synthetic``
...
_METADATA_TYPE_PAIRS: set[tuple[str, str | None]] = {
    ("start", None),
    ("end", None),
    ("today", None),
    ("deviceid", None),
...
def _metadata_kind(question: dict[str, Any]) -> str | None:
...
@dataclass
class StripLog:
    """Collect expressions the synthetic evaluator could not evaluate."""
    ...
    def record(self, var_name: str, kind: str, expr: str, exc: Exception) -> None:
        """Record one expression that failed to evaluate, with its error message."""
        self.entries.append((var_name, kind, expr, str(exc)))

    def write(self, path: Path) -> None:
        """Write the strip-log entries to ``path``."""
        path.parent.mkdir(parents=True, exist_ok=True)
        with path.open("w", encoding="utf-8") as f:
            f.write("# Synthetic-data evaluator strip-log\n")
...
class CoverageTrace:
    ...
    def add_row(self, entry: dict[str, Any]) -> None:
        """Append one per-row coverage entry to the trace."""
        self.rows.append(entry)

    def to_json(self) -> dict[str, Any]:
        """Return the trace as a JSON-serialisable dict with per-variable aggregates."""
        asked: dict[str, int] = {}
        gated: dict[str, int] = {}
        for r in self.rows:
    ...
    def write(self, path: Path) -> None:
        """Write the coverage trace as JSON to ``path``."""
        path.parent.mkdir(parents=True, exist_ok=True)
        with path.open("w", encoding="utf-8") as f:
...
@dataclass
class AnswerResult:
    """One provider's answer for a cell: the value, its source, and an optional note."""
...
class AnswerProvider(Protocol):
    """Protocol for objects that supply an answer for an already-relevant cell."""

    def provide(self, req: AnswerRequest) -> AnswerResult:
        """Return an answer for the requested cell."""
        ...
...
class DefaultStochasticProvider:
    ...
    def provide(self, req: AnswerRequest) -> AnswerResult:
        """Return a stochastic answer sampled for the requested cell."""
        return AnswerResult(
            sample_python_value(
...
def _validate_scripted_value(
    raw: Any, req: AnswerRequest
...
class ScriptedProvider:
    ...
    def __init__(self, answers=None, *, strict_scripted=False, on_result=None):
        """Store the answer sheet, strict-mode flag, and optional result callback."""
    ...
    def provide(self, req: AnswerRequest) -> AnswerResult:
        """Return the scripted answer for the cell, or a sampled fallback if absent/invalid."""
...
def _build_run_context(
    ...
    form_settings: dict[str, Any] | None = None,
) -> RunContext:
    ...
    form_id = (
        (settings.get("form_id") or "synthetic_form").strip()
        if settings
        else "synthetic_form"
    )
    return RunContext(
...
def _build_caseid_pool(
    ...
    tables: dict[str, Any],
    case_id_filter: dict[str, Any] | None = None,
) -> list[str]:
...
def _extract_search_call(appearance: str) -> tuple[str, str] | None:
...
def _resolve_match_value(expr: str, row: dict[str, Any]) -> str | None:
...
#
# This mirrors the resolver in ``surveycto_extractor.cli.vardict._build_repeat_tree``
...
class GateDecision:
    ...
    relevant: bool
    failing_kind: str | None = None  # "group_relevance" | "relevance"
    failing_expr: str | None = None
    failing_operands: dict[str, Any] | None = None  # resolved ${ref} -> value
...
def _is_relevant(
    ...
    * *un-evaluable* -- ``evaluate`` raised ``ExpressionError`` (parse error /
    ...
    def _check(expr: str, kind: str) -> GateDecision | None:
        try:
            v = evaluate(expr, ctx)
...
def _metadata_value(
    q_type: str, q_name: str, runctx: RunContext, rng: random.Random
) -> str:
    if q_type == "start":
...
def _walk_one(
    ...
    rng: random.Random,
    runctx: RunContext,
    pulldata_lookup: Callable[[str, str, str, Any], Any] | None,
    choices_lookup: dict[str, list[dict[str, Any]]] | None,
    var_to_choice_list: dict[str, str] | None,
    strip_log: StripLog,
    force_values: dict[str, str] | None = None,
    geo_bbox: tuple[float, float, float, float] | None = None,
    pulldata_tables: dict[str, Any] | None = None,
    provider: AnswerProvider | None = None,
    strict: bool = True,
    repeat_count_overrides: dict[str, int] | None = None,
    ...
    def _make_ctx(current_var: str | None, repeat_stack: list[tuple[str, int]]):
        return EvalContext(
            row=row,
    ...
    answer_sources: dict[str, tuple[str, str | None]] = {}
    ...
    def _value_for(q, q_type, key, choices, constraint, ctx):
        calc = (
            q.get("calculation") if q_type in ("calculate", "calculate_here") else None
        )
    ...
    def process(detect: bool) -> None:
        ...
        for idx, q in enumerate(questions):
            ...
            if (
                q_type in ("select_one", "select_multiple")
                and pulldata_tables
                and q.get("appearance")
            ):
                dynamic = _resolve_search_choices(
                    q, pulldata_tables, row=row, apply_matches=True
                )
                if dynamic:
                    choices = dynamic
            ...
            if q_type == "repeat_count":
                ...
                for combo in _enumerate_combos(ancestors, counts):
                    ...
                    if _tracing and col in cell_trace and cell_trace[col].get("asked"):
                        cell_trace[col]["source"] = (
                            "directive" if override is not None else "count"
                        )
                continue
    ...
    if _tracing:
        unevaluable = [
            {
                "key": k,
                "expr": c["gate"].get("failing_expr"),
                "error": c["gate"].get("error"),
            }
...
def _compute_value(
    ...
    geo_bbox: tuple[float, float, float, float] | None = None,
    provider: AnswerProvider | None = None,
    answer_sources: dict[str, tuple[str, str | None]] | None = None,
) -> Any:
    ...
    if q_type in ("calculate", "calculate_here"):
        ...
        if not calc:
            return ""
        if q_type == "calculate_here" and re.search(
            r"\bonce\s*\(\s*duration\s*\(", calc
        ):
            remaining = max(0, runctx.duration - runctx.duration_progress)
            advance = (
                max(1, int(remaining * rng.uniform(0.05, 0.20))) if remaining > 0 else 0
            )
    ...
    if provider is None:
        return sample_python_value(
            q_type, narrowed_choices, constraint, rng, **sampler_kwargs
        )
    req = AnswerRequest(
...
def generate_synthetic_csv(
    ...
    strict: bool = True,
    repeat_count_overrides: dict[str, int] | None = None,
    coverage_trace_path: Path | None = None,
    case_id_filter: dict[str, Any] | None = None,
) -> Path:
    ...
    try:
        tables = load_pulldata_tables(
            questions,
            search_dirs=pulldata_search_dirs,
        )
    ...
    caseid_pool = _build_caseid_pool(
        questions, tables or {}, case_id_filter=case_id_filter
    )
    if caseid_pool:
    ...
    def _walk_for_respondent(rs: int, i: int, log: StripLog) -> WalkResult:
        ...
        rctx = _build_run_context(
            meta_rng, i + 1, caseid=caseid_per_row[i], form_settings=form_settings
        )
        return _walk_one(
            questions,
            repeats,
    ...
    column_order = _build_column_order(
        questions, repeats, max_iter, pulldata_tables=tables
    )
