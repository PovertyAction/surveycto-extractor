...
# Sentinel recode table -- shared with vardict.py via core/sentinels.py so the
# mvdecode mapping and the choice-code labels can't drift (#26.8). config (if
# present) may override SENTINEL_MEANINGS; a present-but-broken config raises
# loudly rather than silently reverting to the default recode (#5).
        ...
        rows.append(
            {
                ...
                "relevance": survey.get("skip_logic_iteration_specific")
                or survey.get("stata_skip_logic"),
...
def build_relevance_groups(
    ...
    Args:
        ...
        string_cols: frozenset of string-typed column names. Clauses comparing against
                    these are stripped too, since they cause a Stata r(109) type mismatch.
    ...
    stripped_count = 0
    stripped_examples: list[
        tuple[str, str, str]
    ] = []  # (var, original, cleaned) — up to 3
        ...
        needs_strip = cond and (
            ...
            or (
                ...
                and any(
                    ...
                    for c in _split_top_level_and(cond)
                    if c
                )
            )
        )
    ...
    if stripped_count:
        print(
            f"Conditions with untranslatable clauses stripped: {stripped_count} vars (showing up to 3 examples):"
        )
...
def _chunk_varlist(
    var_names: list[str], limit: int = _STATA_LOCAL_LIMIT
) -> list[list[str]]:
...
def write_destring_block(
    ...
    for i in range(1, len(ds_chunks) + 1):
        ...
        lines.append(
            '        if `"`r(varlist)\'"\' != "" destring `r(varlist)\', replace'
        )
    lines += ["    }", ""]
    ...
    if _sentinels is not None:
        ...
        _override = (
            getattr(_config, "SENTINEL_MEANINGS", None) if _config is not None else None
        )
        _fb_meanings = (
            {int(k): tuple(v) for k, v in _override.items()}
        ...
        _mv = " \\ ".join(
            f"{code}={miss}" for code, (_lbl, miss) in sorted(_fb_meanings.items())
        )
    ...
    for i in range(1, len(mv_chunks) + 1):
        ...
        lines.append(
            f'        if `"`r(varlist)\'"\' != "" mvdecode `r(varlist)\', mv({_mv})'
        )
    lines += ["    }", ""]

    # save cleaned snapshot for skip_cleaning reuse
    lines += [
...
def write_stats_blocks(relevance_groups: dict, zero_groups: set | None = None) -> str:
    r"""Generate Stata code that collects tabstat summary statistics for every
    ...
    if duplicates:
        ...
        print(
            f"WARNING: {len(uniq_dups)} variable(s) appear in multiple "
    ...
    for gi, (cond, chunks) in enumerate(group_data):
        ...
        lines.append(
            f"* --- Group {gi}: {cond_label} | {n_vars} vars, {n_chnks} chunk(s) ---"
        )

        if is_zero:
            n_skipped += 1
            lines.append(
                "* SKIPPED: 0 observations matched this condition in the dataset."
            )
            lines.append(
                "* Uncomment below if data changes and this group gains observations."
            )
            for ci, chunk in enumerate(chunks, 1):
                lines.append(f"* local g{gi}_c{ci} {' '.join(chunk)}")
            for ci in range(1, len(chunks) + 1):
                if_clause = f" if {cond}" if cond else ""
                lines.append(f"* tabstat `g{gi}_c{ci}'{if_clause}, save {_STATS_CMD}")
                if ci == 1:
    ...
    group_markers = [
        i for i, ln in enumerate(code_lines) if ln.startswith("* --- Group ")
    ]
...
def filter_truly_numeric(
    ...
    # -----------------------------------------------------------------------
    str_mask = numeric_universe["stata_type"].str.startswith(
        "str", na=False
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        print(
            "Usage: python load_survey_metadata.py <variable_dictionary.json> <dataset.dta>"
        )
        sys.exit(1)
