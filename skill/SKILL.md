...
- **Shares choices**: same categorical domain — recoding one likely means recoding all
...
4. **Before modifying a variable**: Use `--neighborhood` to see what depends on it
5. **Section exploration**: Use Glob to find section files, then Read specific sections
6. **Multiple variables**: Search efficiently in parallel
