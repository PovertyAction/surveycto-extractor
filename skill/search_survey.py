...
def variable_neighborhood(name: str, depth: int = 1, survey_filter: str = None) -> bool:
    ...
    try:
        ...
        print("ERROR: networkx required for --neighborhood. Install: pip install networkx",
              file=sys.stderr)
        return False
    ...
    for label, files in _filter_surveys(survey_filter).items():
        ...
        if not graph_path.exists():
            ...
            print(f"  Generate it: uv run surveycto-vardict --survey {label}",
                  file=sys.stderr)
            continue
        ...
        sections = [
            ("Calculates from (inputs -- sentinel contamination risk)", "calculates_from"),
            ("Calculated by (downstream -- changing this changes them)", "calculated_by"),
            ("Gated by (missing-by-logic if gate is false)", "gated_by"),
            ("Gates (changing this can make these disappear)", "gates"),
            ("Group-gated by (structural gate -- affects whole group)", "group_gated_by"),
